...
# Accessing clipboard using pyperclip
...
# Reading data from clipboard using pandas
df = pd.read_clipboard()
print(df)

# Writing data to clipboard using pandas
df.to_clipboard()
...
class CustomInstallCommand(Command):
    """Custom install command to run additional tasks."""
...
setup(
    ...
    cmdclass={
        'install': CustomInstallCommand,  # Overwriting the 'install' command
    },
)
...
suspicious_url = "http://example.xyz/path/to/resource"

# Another example of a suspicious URL
another_suspicious_url = "https://bit.ly/shortened"

# Example of a local IP address (should be excluded)
local_ip_url = "http://192.168.1.1"

# Example of a public IP address (should be excluded)
public_ip_url = "http://8.8.8.8"

# Example of a cloud provider metadata service IP (should be excluded)
cloud_metadata_url = "http://169.254.169.254/latest/meta-data/"

# Example of a Discord URL (should be excluded)
